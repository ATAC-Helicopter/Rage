...
class VulkanConfiguration:
    requiredVulkanVersion = "1.4.350.0"
    ...
    def CheckVulkanSDK(cls):
        ...
        if cls.requiredVulkanVersion not in vulkanSDK:
            print(
                f"You don't have the correct Vulkan SDK version! "
    ...
    def __InstallVulkanSDK(cls):
        ...
        vulkanInstallURL = (
            f"https://sdk.lunarg.com/sdk/download/{cls.requiredVulkanVersion}/windows/"
            f"VulkanSDK-{cls.requiredVulkanVersion}-Installer.exe"
        )

        vulkanPath = (
            f"{cls.vulkanDirectory}/"
            f"VulkanSDK-{cls.requiredVulkanVersion}-Installer.exe"
        )
    ...
    def CheckVulkanSDKDebugLibs(cls):
        # Bypass debug library download.
        # The original repo tries to download Vulkan debug libs from LunarG,
        # but the downloaded file is currently not being read as a valid zip.
        # This allows Premake/project generation to continue.
        return True
